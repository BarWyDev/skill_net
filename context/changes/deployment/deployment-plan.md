...
| Workers plan | **Workers Paid ($5/mo)** before the first deploy | Free limits (10 ms CPU, 50 subrequests) break crisis fan-out. |
...
| 8 | *(optional, later)* Custom domain | ⬜ | Not in this release |
...
**Accounts (all `[human]`):**
- [ ] Cloudflare account. **Upgrade to Workers Paid** (Dashboard → Workers & Pages → Plans).
- [ ] **Register the workers.dev subdomain** (Dashboard → Workers & Pages → the onboarding sets `<subdomain>.workers.dev`). Write the chosen `<subdomain>` here: `____`.
  - *Edge case:* if it isn't registered, `wrangler deploy` tries an interactive prompt. That fails in Workers Builds with "cannot be run in a non-interactive context".
...
- [ ] `[agent]` Verify with `npx wrangler whoami`. Record the **Account ID** in this plan (it isn't a secret): `____`.
  - *Edge case:* if more than one account is listed, the agent sets `CLOUDFLARE_ACCOUNT_ID` in the shell (or `account_id` in `wrangler.jsonc`) so commands never prompt.
...
- [ ] *(optional)* `[human]` Add the Cloudflare MCP servers (Observability, Builds) to the user-level Claude config, never to a committed `.mcp.json`. The CLI alone is enough for this plan.
...
1. [ ] **Rename to `skillnet`.** Set `"name": "skillnet"` in `wrangler.jsonc` and `package.json`, then run `npm install --package-lock-only` so `package-lock.json` matches.
   - *Why:* Workers Builds fails if the dashboard Worker name ≠ the wrangler config name.
2. [ ] **Pin the `SESSION` KV namespace.** The adapter injects an ID-less `SESSION` binding (`node_modules/@astrojs/cloudflare/dist/wrangler.js`, `cloudflareConfigCustomizer`). Wrangler then auto-provisions it, and Workers Builds re-provisioning has failed with **error 10014, "namespace already exists"** (workers-sdk #14284/#14262, astro #15802).
...
3. [ ] **Tighten `wrangler.jsonc`:**
   - `"workers_dev": true` and `"preview_urls": true`, set explicitly.
   - `"limits": { "cpu_ms": 30000 }`, as the risk register recommends. It also acts as a guard: if deploy rejects it, the account isn't on Paid, so stop.
   - Keep `observability.enabled: true`. Add a CLAUDE.md rule: no personal data in `console.*`.
4. [ ] **Set `site` in `astro.config.mjs`** to `https://skillnet.<subdomain>.workers.dev`. `@astrojs/sitemap` silently skips generation without `site`.
5. [ ] **Read-only smoke mode.** In `scripts/smoke.mjs`, when `SMOKE_READONLY=1`, run only the first two steps ("home renders" and "dashboard redirects anonymous user").
   - *Why:* production has email confirmation on (signup can't sign in immediately), and the full smoke would litter production `auth.users` with `smoke-*@example.com` accounts.
   - Leave the default (full) mode untouched so the CI `smoke` job is unchanged.
6. [ ] **Local verification:** `npx astro sync && npm run lint && npx astro check && npm run build`, then `npx wrangler deploy --dry-run`. Confirm in the output:
...
- [ ] `[human]` Auth → URL Configuration:
  ...
  - *Edge case:* without this, confirmation emails link to `localhost:3000`, which is the `supabase/config.toml` default.
...
1. [ ] `[agent]` `git init -b master`.
   ...
   - First commit.
2. [ ] `[approve]` `gh repo create skillnet --<private|public> --source . --push`. This publishes code to GitHub.
3. [ ] `[human]` Add the GitHub repo secrets `SUPABASE_URL` / `SUPABASE_KEY`, which the existing `ci` job's build step uses. Use the prod anon values or dummies; the build doesn't need real ones.
4. [ ] `[human]` Cloudflare Dashboard → Workers & Pages → **skillnet** → Settings → Builds → **Connect** to the GitHub repo (install the Cloudflare GitHub App on this repo only). Configure:
   ...
   - **Non-production branch deploy command: `npx wrangler versions upload`.** Do *not* keep the new default `npx wrangler preview`, which needs wrangler ≥ 4.135.0; the project pins 4.131.1.
...
- [ ] `[human]` Worker → Settings → Domains & Routes → **Preview URLs → enable Cloudflare Access** (one click). Allow only your email.
  - *Why:* previews run against the **production** Supabase.
...
- [ ] `[human]` GitHub branch protection / ruleset on `master`: require the status checks `ci` and `smoke`, and require a PR before merging.
  - *Why:* Workers Builds deploys on **every push to `master`**, regardless of GitHub Actions results. This is the only thing that stops a red build shipping.
  - *Edge case:* rulesets and branch protection on **private** repos need GitHub Pro/Team. If the repo is private on Free, the fallback is discipline: always merge through a PR after green CI. Note this as an accepted risk.
...
- [ ] `[approve]` **Rollback rehearsal.**
  ...
  - *Note:* the next push to `master` redeploys over any rollback, so after a real rollback also revert the commit.
...
- [ ] *(optional)* `/10x-lesson` for any class of failure hit during this run.
...
## Phase 8 — *(optional, later)* Custom domain ⬜
